...
class ReleaseWorkspaceUpdateContributingWorkspacesTest(TestCase):
    ...
    def test_can_update_one_upload_workspace(self):
        ...
        self.assertEqual(self.release_workspace.contributing_upload_workspaces.count(), 1)
        self.assertIn(self.upload_workspace, self.release_workspace.contributing_upload_workspaces.all())
        # History is added.
    ...
    def test_can_update_two_upload_workspaces(self):
        ...
        self.assertEqual(models.ReleaseWorkspace.objects.count(), 1)
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_upload_workspaces.count(), 2)
        self.assertIn(self.upload_workspace, self.release_workspace.contributing_upload_workspaces.all())
        self.assertIn(upload_workspace_2, self.release_workspace.contributing_upload_workspaces.all())
    ...
    def test_can_update_one_dcc_processed_data_workspace(self):
        ...
        self.assertEqual(models.ReleaseWorkspace.objects.count(), 1)
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_dcc_processed_data_workspaces.count(), 1)
        self.assertIn(
            dcc_processed_data_workspace, self.release_workspace.contributing_dcc_processed_data_workspaces.all()
        )
        # History is added.
        self.assertEqual(self.release_workspace.history.count(), 2)
        self.assertEqual(self.release_workspace.history.latest().history_type, "~")
    ...
    def test_can_update_one_partner_upload_workspace(self):
        ...
        self.assertEqual(models.ReleaseWorkspace.objects.count(), 1)
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_partner_upload_workspaces.count(), 1)
        self.assertIn(partner_upload_workspace, self.release_workspace.contributing_partner_upload_workspaces.all())
        # History is added.
        self.assertEqual(self.release_workspace.history.count(), 2)
        self.assertEqual(self.release_workspace.history.latest().history_type, "~")
    ...
    def test_can_update_two_partner_upload_workspaces(self):
        ...
        self.assertEqual(models.ReleaseWorkspace.objects.count(), 1)
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_partner_upload_workspaces.count(), 2)
    ...
    def test_invalid_upload_workspace_wrong_consent_group(self):
        ...
        response = self.client.post(
            self.get_url(self.release_workspace.workspace.billing_project.name, self.release_workspace.workspace.name),
            {
                "contributing_upload_workspaces": [upload_workspace.pk],
            },
        )
        self.assertEqual(response.status_code, 200)
        form = response.context_data["form"]
        ...
        self.assertIn("contributing_upload_workspaces", form.errors.keys())
        self.assertEqual(len(form.errors["contributing_upload_workspaces"]), 1)
        self.assertIn(form.ERROR_DIFFERENT_CONSENT_GROUP, form.errors["contributing_upload_workspaces"][0])
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_upload_workspaces.count(), 0)

    def test_invalid_dcc_processed_data_workspace_wrong_consent_group(self):
        """Cannot add a workspace with a different consent group."""
        dcc_processed_data_workspace = factories.DCCProcessedDataWorkspaceFactory.create()
        self.client.force_login(self.user)
        response = self.client.post(
        ...
        self.assertFalse(form.is_valid())
        self.assertEqual(len(form.errors.keys()), 1)
        self.assertIn("contributing_dcc_processed_data_workspaces", form.errors.keys())
        self.assertEqual(len(form.errors["contributing_dcc_processed_data_workspaces"]), 1)
        self.assertIn(form.ERROR_DIFFERENT_CONSENT_GROUP, form.errors["contributing_dcc_processed_data_workspaces"][0])
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_upload_workspaces.count(), 0)
        self.assertEqual(self.release_workspace.contributing_dcc_processed_data_workspaces.count(), 0)

    def test_invalid_partner_upload_workspace_wrong_consent_group(self):
        """Cannot add a workspace with a different consent group."""
        partner_upload_workspace = factories.PartnerUploadWorkspaceFactory.create()
        self.client.force_login(self.user)
        response = self.client.post(
        ...
        self.assertFalse(form.is_valid())
        self.assertEqual(len(form.errors.keys()), 1)
        self.assertIn("contributing_partner_upload_workspaces", form.errors.keys())
        self.assertEqual(len(form.errors["contributing_partner_upload_workspaces"]), 1)
        self.assertIn(form.ERROR_DIFFERENT_CONSENT_GROUP, form.errors["contributing_partner_upload_workspaces"][0])
        self.release_workspace.refresh_from_db()
        self.assertEqual(self.release_workspace.contributing_upload_workspaces.count(), 0)
    ...
    def test_initial_no_contributing_workspaces_suggests_rc_processed_data(self):
        ...
        self.assertIn(rc_processed_data_workspace, form.initial["contributing_rc_processed_data_workspaces"])

    def test_initial_no_contributing_workspaces_rc_processed_data_workspace_same_consent_group(self):
        """Only suggests partner workspaces with the same consent group."""
        rc_processed_data_workspace = factories.RCProcessedDataWorkspaceFactory.create(
            date_completed=timezone.now().date() - timezone.timedelta(days=1),
        )
        ...
        self.assertNotIn(rc_processed_data_workspace, form.initial["contributing_rc_processed_data_workspaces"])

    def test_initial_no_contributing_workspaces_rc_processed_data_same_only_completed(self):
        """Only suggests RC processed data workspaces that are completed."""
        rc_processed_data_workspace = factories.RCProcessedDataWorkspaceFactory.create(
            consent_group=self.release_workspace.consent_group,
            date_completed=None,
        )
    ...
    def test_initial_no_contributing_workspaces_rc_processed_data_highest_version(self):
        """Only suggests the most highest version of all completed RC processed data workspaces for a given RC."""
        research_center = factories.ResearchCenterFactory.create()
        rc_processed_data_workspace_1 = factories.RCProcessedDataWorkspaceFactory.create(
        ...
        self.assertIn("contributing_rc_processed_data_workspaces", form.initial)
        self.assertEqual(len(form.initial["contributing_rc_processed_data_workspaces"]), 1)
        self.assertNotIn(rc_processed_data_workspace_1, form.initial["contributing_rc_processed_data_workspaces"])
        self.assertIn(rc_processed_data_workspace_2, form.initial["contributing_rc_processed_data_workspaces"])

    def test_initial_no_contributing_workspaces_suggests_rc_processed_data_workspaces_two_rcs(self):
    ...
    def test_initial_with_contributing_workspaces_rc_processed_data_not_set(self):
        """Initial is not set when workspace already has contributing workspaces saved."""
        contributing_workspace = factories.UploadWorkspaceFactory.create()
        rc_processed_data_workspace = factories.RCProcessedDataWorkspaceFactory.create(
            consent_group=self.release_workspace.consent_group,
...
class DCCProcessedDataWorkspaceSharingAuditTest(AnVILAPIMockTestMixin, TestCase):
    """Tests for the DCCProcessedDataWorkspaceSharingAudit view."""
    ...
    def get_url(self, *args):
        """Get the url for the view being tested."""
        return reverse(
            "gregor_anvil:audit:dcc_processed_data_workspaces:sharing:all",
            args=args,
        )

    def get_view(self):
        """Return the view being tested."""
        return views.DCCProcessedDataWorkspaceSharingAudit.as_view()
    ...
    def test_context_audit_results_two_workspaces(self):
        """The audit_results exists in the context."""
        workspace_1 = factories.DCCProcessedDataWorkspaceFactory.create()
        workspace_2 = factories.DCCProcessedDataWorkspaceFactory.create()
        self.client.force_login(self.user)
        response = self.client.get(self.get_url())
        self.assertIn("audit_results", response.context_data)
        audit_results = response.context_data["audit_results"]
    ...
    def test_context_verified_table_access(self):
        ...
        self.assertEqual(table.rows[0].get_cell_value("workspace"), workspace.workspace)
        self.assertEqual(table.rows[0].get_cell_value("managed_group"), group)
        self.assertEqual(table.rows[0].get_cell_value("access"), acm_models.WorkspaceGroupSharing.READER)
        self.assertEqual(
            table.rows[0].get_cell_value("note"),
...
class DCCProcessedDataWorkspaceSharingAuditByUploadCycleTest(AnVILAPIMockTestMixin, TestCase):
    """Tests for the DCCProcessedDataWorkspaceSharingAuditByUploadCycle view."""
    ...
    def get_url(self, *args):
        """Get the url for the view being tested."""
        return reverse(
            "gregor_anvil:audit:dcc_processed_data_workspaces:sharing:by_upload_cycle",
            args=args,
        )

    def get_view(self):
        """Return the view being tested."""
        return views.DCCProcessedDataWorkspaceSharingAuditByUploadCycle.as_view()
    ...
    def test_context_audit_results_two_workspaces(self):
        """The audit_results exists in the context."""
        workspace_1 = factories.DCCProcessedDataWorkspaceFactory.create(upload_cycle=self.upload_cycle)
        workspace_2 = factories.DCCProcessedDataWorkspaceFactory.create(upload_cycle=self.upload_cycle)
        self.client.force_login(self.user)
        response = self.client.get(self.get_url(self.upload_cycle.cycle))
        self.assertIn("audit_results", response.context_data)
        audit_results = response.context_data["audit_results"]
    ...
    def test_context_audit_results_ignores_other_upload_cycles(self):
        """The audit_results exists in the context."""
        workspace = factories.DCCProcessedDataWorkspaceFactory.create(upload_cycle__cycle=self.upload_cycle.cycle + 1)
        self.client.force_login(self.user)
        response = self.client.get(self.get_url(self.upload_cycle.cycle))
        self.assertIn("audit_results", response.context_data)
        audit_results = response.context_data["audit_results"]
        ...
        self.assertTrue(audit_results.completed)
        self.assertEqual(audit_results.queryset.count(), 0)
        self.assertNotIn(workspace, audit_results.queryset)
    ...
    def test_context_verified_table_access(self):
        ...
        group = acm_factories.ManagedGroupFactory.create(name=settings.ANVIL_DCC_ADMINS_GROUP_NAME)
        acm_factories.WorkspaceGroupSharingFactory.create(
            workspace=workspace.workspace, group=group, access=acm_models.WorkspaceGroupSharing.OWNER
        )
        # Check the table in the context.
        self.client.force_login(self.user)
        response = self.client.get(self.get_url(self.upload_cycle.cycle))
...
class DCCProcessedDataWorkspaceSharingAuditResolveTest(AnVILAPIMockTestMixin, TestCase):
    def setUp(self):
        """Set up test class."""
        super().setUp()
        self.factory = RequestFactory()
    ...
    def get_view(self):
        """Return the view being tested."""
        return views.DCCProcessedDataWorkspaceSharingAuditResolve.as_view()
    ...
    def test_post_share_as_reader_htmx(self):
        ...
        self.assertEqual(response.content.decode(), views.DCCProcessedDataWorkspaceSharingAuditResolve.htmx_success)
        self.assertEqual(acm_models.WorkspaceGroupSharing.objects.count(), 1)
        sharing = acm_models.WorkspaceGroupSharing.objects.get(workspace=workspace.workspace, group=group)
        self.assertEqual(sharing.access, acm_models.WorkspaceGroupSharing.READER)
        self.assertFalse(sharing.can_compute)
    ...
    def test_post_share_as_reader_anvil_api_error_htmx(self):
        ...
        self.assertEqual(response.content.decode(), views.DCCProcessedDataWorkspaceSharingAuditResolve.htmx_error)
        # No sharing object was created.
        self.assertEqual(acm_models.WorkspaceGroupSharing.objects.count(), 0)
        # No messages were added.
        messages = [m.message for m in get_messages(response.wsgi_request)]
    ...
    def test_post_new_share_with_compute_group_not_found_on_anvil_htmx(self):
        group = acm_factories.ManagedGroupFactory.create(name="GREGOR_DCC_WRITERS")
        workspace = factories.DCCProcessedDataWorkspaceFactory.create(
            workspace__billing_project__name="test-bp",
            workspace__name="test-ws",
...
class DCCProcessedDataWorkspaceAuthDomainAuditTest(AnVILAPIMockTestMixin, TestCase):
    """Tests for the DCCProcessedDataWorkspaceSharingAudit view."""
    ...
    def get_url(self, *args):
        """Get the url for the view being tested."""
        return reverse(
            "gregor_anvil:audit:dcc_processed_data_workspaces:auth_domains:all",
            args=args,
        )

    def get_view(self):
        """Return the view being tested."""
        return views.DCCProcessedDataWorkspaceAuthDomainAudit.as_view()
    ...
    def test_context_verified_table_access(self):
        ...
        self.assertIsInstance(
            table,
            dcc_processed_data_workspace_audit.DCCProcessedDataWorkspaceAuthDomainAuditTable,
        )
        self.assertEqual(len(table.rows), 1)
        self.assertEqual(table.rows[0].get_cell_value("workspace"), workspace.workspace)
        self.assertEqual(table.rows[0].get_cell_value("managed_group"), group)
    ...
    def test_context_verified_table_no_access(self):
        """verified_table shows a record when audit has verifiednotmember."""
        workspace = factories.DCCProcessedDataWorkspaceFactory.create(upload_cycle__is_past=True)
        # Create and share a combined workspace.
        factories.CombinedConsortiumDataWorkspaceFactory(
            upload_cycle=workspace.upload_cycle,
            date_completed=timezone.localdate() - timedelta(days=1),
...
class DCCProcessedDataWorkspaceAuthDomainAuditByWorkspaceTest(AnVILAPIMockTestMixin, TestCase):
    """Tests for the DCCProcessedDataWorkspaceAuthDomainAuditByWorkspace view."""
    ...
    def get_view(self):
        """Return the view being tested."""
        return views.DCCProcessedDataWorkspaceAuthDomainAuditByWorkspace.as_view()
    ...
    def test_context_verified_table_no_access(self):
        """verified_table shows a record when audit has verifiednotmember."""
        self.workspace.upload_cycle.start_date = timezone.localdate() - timedelta(days=20)
        self.workspace.upload_cycle.start_date = timezone.localdate() - timedelta(days=10)
        self.workspace.upload_cycle.save()
        # Create and share a combined workspace.
        factories.CombinedConsortiumDataWorkspaceFactory(
            upload_cycle=self.workspace.upload_cycle,
            date_completed=timezone.localdate() - timedelta(days=1),
...
class DCCProcessedDataWorkspaceAuthDomainAuditByUploadCycleTest(AnVILAPIMockTestMixin, TestCase):
    """Tests for the DCCProcessedDataWorkspaceSharingAuditByUploadCycle view."""
    ...
    def get_url(self, *args):
        """Get the url for the view being tested."""
        return reverse(
            "gregor_anvil:audit:dcc_processed_data_workspaces:auth_domains:by_upload_cycle",
            args=args,
        )

    def get_view(self):
        """Return the view being tested."""
        return views.DCCProcessedDataWorkspaceAuthDomainAuditByUploadCycle.as_view()
